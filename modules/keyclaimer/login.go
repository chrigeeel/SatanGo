...
func login(profiles []loader.ProfileStruct, site siteStruct) []loader.ProfileStruct {
	...
	loginLocal := func(wg *sync.WaitGroup, id int) {
		...
		rurl := "https://discord.com/api/v9/oauth2/authorize?client_id=" + site.ClientID + "&response_type=code&redirect_uri=" + url.QueryEscape(site.URL) + "login%2Fcallback&scope=identify%20email%20guilds.join%20guilds"
		...
		callBackUrl := site.URL + "login/callback?code=" + code
