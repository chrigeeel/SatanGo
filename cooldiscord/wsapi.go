...
func (s *Session) Open() error {
	...
	if err != nil {
		s.log(LogInformational, "error connecting to gateway %s, %s", s.gateway, err)
...
func (s *Session) heartbeat(wsConn *websocket.Conn, listening <-chan interface{}, heartbeatIntervalMsec time.Duration) {
	...
	for {
		...
		if err != nil || time.Now().UTC().Sub(last) > (heartbeatIntervalMsec*FailedHeartbeatAcks) {
			if err != nil {
				s.log(LogInformational, "error sending heartbeat to gateway %s, %s", s.gateway, err)
			} else {
				s.log(LogInformational, "haven't gotten a heartbeat ACK in %v, triggering a reconnection", time.Now().UTC().Sub(last))
			}
			s.Close()
			s.reconnect()
			return
...
func (s *Session) onEvent(messageType int, message []byte) (*Event, error) {
	...
	if messageType == websocket.BinaryMessage {
		...
		if err2 != nil {
			s.log(LogInformational, "error uncompressing websocket message, %s", err)
			return nil, err2
		}
	...
	if err = decoder.Decode(&e); err != nil {
		s.log(LogInformational, "error decoding websocket message, %s", err)
		return e, err
	}
	...
	if e.Operation == 1 {
		...
		s.wsMutex.Unlock()
		if err != nil {
			s.log(LogInformational, "error sending heartbeat in response to Op1")
			return e, err
		}
	...
	if eh, ok := registeredInterfaceProviders[e.Type]; ok {
		...
		if err = json.Unmarshal(e.RawData, e.Struct); err != nil {
			//s.log(LogInformational, "error unmarshalling %s event, %s", e.Type, err)
		}
...
func (s *Session) onVoiceServerUpdate(st *VoiceServerUpdate) {
	...
	err := voice.open()
	if err != nil {
		s.log(LogInformational, "onVoiceServerUpdate voice.open, %s", err)
	}
}
...
func (s *Session) reconnect() {
	...
	if s.ShouldReconnectOnError {
		...
		for {
			...
			s.log(LogInformational, "error reconnecting to gateway, %s", err)
